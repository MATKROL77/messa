...
export default function MesaPage() {
  ...
  return (
    <div className="mesa-experience">
      ...
      {vista === 'pago' && !esModoStaff && <PagoView pedidos={pedidosEnCocina} propinaConfig={propinaConfig} pagoCompletado={pagoCompletado} setPagoCompletado={setPagoCompletado} onVolver={() => setVista('menu')} sesionDispositivoId={sesion?.dispositivo_id || ''} mesaId={mesaId} mesaNumero={mesaNumero} config={config} marcarComoPagado={marcarComoPagado} prepararPagoManual={prepararPagoManual} setModoPostPago={setModoPostPago} irAResenas={() => setVista('reviews')} onLlamarMozo={handleLlamarMozo} emailPago={emailPago} setEmailPago={setEmailPago} />}
...
function PagoView({ pedidos, propinaConfig, pagoCompletado, setPagoCompletado, onVolver, sesionDispositivoId, mesaId, mesaNumero, config, marcarComoPagado, prepararPagoManual, setModoPostPago, irAResenas, onLlamarMozo, emailPago, setEmailPago }: PagoViewProps) {
      ...
      {todosItems.length === 0 ? <div className="mesa-empty-selection"><ReceiptText size={24} /><h3>No hay consumos pendientes</h3><p>Cuando envíes un pedido a cocina, su resumen aparecerá acá.</p></div> : (
        <>
          ...
          <div className="mesa-payment-options">
            {[{ v: 'total', l: 'Pagar todo', s: formatPrecio(totalGeneral), Icon: WalletCards }, { v: 'dividido', l: 'Solo lo mío', s: formatPrecio(miTotal), Icon: UserRound }].map(o => (
              <button type="button" key={o.v} className={metodoPago === o.v ? 'active' : ''} onClick={() => setMetodoPago(o.v as 'total' | 'dividido')} aria-pressed={metodoPago === o.v}>
                <o.Icon size={18} />
                <p>{o.l}</p>
